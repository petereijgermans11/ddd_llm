...
from ddd_llm_app.config.settings import OPENAI_API_KEY, OPENAI_MODEL
...
class OpenAIPromptBuilder(PromptBuilder):
    ...
    def template_file(self, templ_file: str) -> 'OpenAIPromptBuilder':
        self._templ_file = templ_file
        return self

    def domain_description_file(self, dom_desc_file: str) -> 'OpenAIPromptBuilder':
        self._dom_desc_file = dom_desc_file
        return self

    def domain_description_text(self, dom_desc_text: str) -> 'OpenAIPromptBuilder':
...
class OpenAIProvider(BaseProvider):
    def __init__(self):
        self.llm = ChatOpenAI(
            ...
            max_tokens= Config.config['openai']['max_tokens'], #8192,
            temperature=Config.config['openai']['temperature'], #0.7
        )
    ...
    def createPromptBuilder(self,config: Config = None) -> PromptBuilder:
        return OpenAIPromptBuilder(config)
